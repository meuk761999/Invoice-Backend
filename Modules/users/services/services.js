...
const {JWT_SECRET,JWT_HEADER_KEY} = require("../../../config");
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (req.body) {
        ...
        bcrypt.genSalt(10,(err,salt)=>bcrypt.hash(newUser.userPassword,salt,async (err,hash)=>
        {
          if(err)
          return reject({
            status: 500,
    ...
    try {
      bcrypt.compare(req.body.userPassword,req.query.userPassword,(err,result)=>{
        ...
        else if(result)
        {
          ...
          jwt.sign({tokenData},JWT_SECRET,{expiresIn:'1h'},(err,jWtoken)=>{
            if(err){
              return reject({
                status: 500,
